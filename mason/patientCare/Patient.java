...
public class Patient implements Steppable {
	...
	protected void expectationFormation(Care care) {
		...
		for(int w = 0; w < care.W; w++) {
			e_fluctuation = care.random.nextGaussian()*(rho_p + eta_p)*kappa_p;
	...
	public void stepForwardStateVariables(){
		...
		e_p_i_1 = e_p_i;
			for(int i = 0; i< e_p_i.length;i++) {e_p_i[i]=0;}
		c_p_i_1 = c_p_i;
			for(int i = 0; i< c_p_i.length;i++) {c_p_i[i]=0;}
		b_p_i_1 = b_p_i;
			for(int i = 0; i< b_p_i.length;i++) {b_p_i[i]=0;}
	}	
}
