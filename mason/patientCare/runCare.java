...
public class runCare {
	...
	public static void main(String[] args) {
		if(args.length == 0) {System.out.println("path missing");return;}
		...
		String[] pars = {"motivation", "severity", "capacity", "numPatients"};
		...
		for(int i = 0; i < args.length; i++) { //loops through args to find parameters
			if(args[i].equals(pars[a])) {
				...
				} catch (NumberFormatException e){System.out.println("Invalid parameter "+ pars[a]);return;}
				switch(pars[a]) { // adds the parameters to simulation
				case "motivation":
					simulation.setMotivationTrheshold(Double.valueOf(args[i+1]));
					break;
				case "severity":
					simulation.setSeverityTrheshold(Double.valueOf(args[i+1]));
					break;
				case "capacity":
