 ...
 * STEPS: Step 0 is for the observer. Step 1 for the providers. From step 2
 * onwards all patients. An anonymus agents is scheduled last that shcedules all
 * patients.
 */
public class Care extends SimState {
	...
	public double prioritization_granularity = -1;
	/**
	...
	/**
	 * Generates an observer for the specified variables and scheddules it with
	 * priority 0
	 * 
	 * @param obsH
	 * @param obsN
	...
	public void startObserver(boolean obsH, boolean obsN, boolean obsC,
			boolean obsT, boolean obsE, boolean obsB, boolean simpleC, boolean simpleE, boolean simpleB,
			boolean disease, boolean expNoise, boolean instExp, boolean delta, boolean performance, boolean maxExp) {
		startObserver(obsH, obsN, obsC, obsT, obsE, obsB, simpleC, simpleE, simpleB, disease, expNoise, instExp, delta,
				performance, maxExp, false, false);
	}

	public void startObserver(boolean obsH, boolean obsN, boolean obsC,
			boolean obsT, boolean obsE, boolean obsB, boolean simpleC, boolean simpleE, boolean simpleB,
			boolean disease, boolean expNoise, boolean instExp, boolean delta, boolean performance) {
		startObserver(obsH, obsN, obsC, obsT, obsE, obsB, simpleC, simpleE, simpleB, disease, expNoise, instExp, delta,
				performance, false);
	}
	...
	public void startObserver(boolean obsH, boolean obsN, boolean obsC,
		...
		startObserver(obsH, obsN, obsC, obsT, obsE, obsB, simpleC, simpleE, simpleB, disease, expNoise, instExp, false,
				false, false);
	}
	...
	/**
	 * Initializes initializers (if not previously done). Initializes the
	 * Prioritizator.
	 * SEED: It re-sets the seed after initializers configuration to keep
	 * consistency of replication. Its re-set again after agents initialization.
	 * Creates and initializes and schedules providers and patients.
	 * Creates anonymous agent that implements the policy. This is the last agent to
	 * called at the end of each step.
	 */
	public void start() {
		super.start();
		if (pat_init == null) {
			// System.out.println("(JAVA Care.java) Configure patient initialization to
			// "+PATIENT_INIT);
			pat_init = new PatientInitializer(this, PATIENT_INIT);
		}
		// else {System.out.println("(JAVA Care.java) Patient initialization
		// pre-configured");}
		if (prov_init == null) {
			// System.out.println("(JAVA Care.java) Configure provider initialization to
			// "+PROVIDER_INIT);
			prov_init = new ProviderInitializer(this, PROVIDER_INIT);
		} else {
			// System.out.println("(JAVA Care.java) Provider initialization
			// pre-configured");
		}

		prioritize = new Prioritizator(this, Pi);
		if (prioritization_granularity != -1) {
			prioritize.setGranularity(prioritization_granularity);
		}
		if (OBS_PERIOD == 0) {
			OBS_PERIOD = varsigma;
		}
		...
		// INITIALIZING AGENTS (configuring the initializers sometimes uses random
		// number generator)
	...
	public void setvarsigma(int val) {
		varsigma = val;
	}

	public int getvarsigma() {
		return varsigma;
	}
	...
	public void setPi(String val) {
		...
		// Pi = val;
		if (val.equals("basal") || val.equals("H_segmented") || val.equals("patient_centred") || val.equals("risk")
				|| val.equals("need") || val.equals("risk_need")) {
			Pi = val;
		} else {
	...
	public void setPROVIDER_INIT(String val) {
		PROVIDER_INIT = val;
	}

	public String getPROVIDER_INIT() {
		return PROVIDER_INIT;
	}

	public void setPATIENT_INIT(String val) {
		PATIENT_INIT = val;
	}

	public String getPATIENT_INIT() {
		return PATIENT_INIT;
	}

	public void setOBS_PERIOD(int val) {
		OBS_PERIOD = val;
	}
	...
	public void settotalCapacity(int val) {
		totalCapacity = val;
	}
	...
	public HashMap<String, String> getParams() {
		...
		params.put("prioritization_granularity", Double.toString(prioritization_granularity));
		// Patient level
	...
	public void change_W_midwaytrhough(int newW) {
		...
		if (newW < W) {
			...
			for (int i = 0; i < W - newW; i++) {
				...
				// System.out.println("(Care) Observer contains:
				// observer.B_p_w_i[5][goneProdiver][25]"+observer.B_p_w_i[5][provider.w][25]);
			}
			// System.out.println();
			prov_init.adjustCapacity(providers, totalCapacity);
	 ...
	 * Only way I came up with to see the order that the scheduler assigned to
	 * agents with same priority.
